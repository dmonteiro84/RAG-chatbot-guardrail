...
def analyze_results(data, evaluation_type):
    ...
    - For 'language_check', compute and display Confusion Matrix for language predictions.
    """
    ...
    if evaluation_type == "topic_filter":
    ...
    elif evaluation_type == "language_check":
        y_true = []
        y_pred = []

        for entry in data:
            # Ground truth and predicted outcome for language check
            y_true.append(entry["not_english_actual"])
            y_pred.append(entry["not_english_predicted"])
        ...
        plt.title("Language Check Confusion Matrix (Percentages)", fontsize=18)
        plt.xlabel("Predicted", fontsize=16)
        plt.ylabel("Actual", fontsize=16)
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
...
def main():
    ...
    parser.add_argument("--type", type=str, required=True, choices=["topic_filter", "groundedness", "language_check"], help="Type of evaluation: 'topic_filter', 'groundedness', or 'language_check'")
